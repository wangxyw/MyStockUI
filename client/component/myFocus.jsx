import { Table, ArrowUpOutlined } from 'antd';
...
const columns = [
    {
      ...
      dataIndex: 'symbol',
      key: 'symbol',
      render: (text) => {return <a target="_blank" href={`https://finance.sina.com.cn/realstock/company/${text}/nc.shtml`}>{text}</a>}
    }, {
        title: 'Name',
        dataIndex: 'name',
        key: 'name'
        ...
        render: (txt) => {if(txt === 'Up') {return '看涨'} if(txt === 'Down') {return '看跌'}}
      },
      {
        title: 'Final Price',
...
export const MyFocusListComponent = () => {
    ...
    useEffect(()=>{
        fetch('/all_focus_stock').then(res => res.json()).then(data => {
            setData(data);
        })
    }, [])
    return (
