...
export const DataAlarmCom = (props) => {
  const { isDR } = props;
  ...
  const [data400s, setData400s] = useState<any>();
  const [data400dr, setData400dr] = useState<any>();

  const [data100w, setData100w] = useState<any>();

  const [data100dr, setData100dr] = useState<any>();
  ...
  const runAnalysis = () => {
    setIsLoading(true);
    let days = parseInt(selectConsAllDays, 10);
    ...
    get(
      ...
      ['400s', '100w', 'dr_400s', 'dr_100w'].forEach((from) => {
          ...
          {isSetCondition && data && (
            <div>
              ...
              <Table
                ...
                rowClassName={(record: any) => {
                  if (record?.isNotFirst) {
                    return 'da-row red-row-first s-row';
                  }
                  if (record?.isJustFirst) {
                    return 'da-row just-row-first s-row';
                  }
                  return 'da-row s-row';
                }}
              />
              ...
              <Table
                ...
                rowClassName={(record: any) => {
                  if (record?.isNotFirst) {
                    return 'da-row red-row-first w-row';
                  }
                  if (record?.isJustFirst) {
                    return 'da-row just-row-first w-row';
                  }
                  return 'da-row w-row';
                }}
              />
              ...
              <Table
                ...
                rowClassName={(record: any) => {
                  if (record?.isNotFirst) {
                    return 'da-row red-row-first sdr-row';
                  }
                  if (record?.isJustFirst) {
                    return 'da-row just-row-first sdr-row';
                  }
                  return 'da-row sdr-row';
                }}
              />
              <h1>100wDR:</h1>
              <Table
                ...
                rowClassName={(record: any) => {
                  if (record?.isNotFirst) {
                    return 'da-row red-row-first wdr-row';
                  }
                  if (record?.isJustFirst) {
                    return 'da-row just-row-first wdr-row';
                  }
                  return 'da-row wdr-row';
                }}
              />
              todo: 1.在哪里已经出现过 2.60天内没出现过
            </div>
          )}
          {/* {data && (
            <Table
