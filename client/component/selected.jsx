...
export class Selected extends React.Component {
    ...
    return (
      <section className="pageContentInner">
        <div className="head-section">
          <h1>Stock ID: {this.state.selectStockId}</h1>
        </div>
        ...
        <Select
          ...
          style={{ width: '200px' }}
          onChange={(v) => {
            self.setState({ selectStockId: v });
            self.reDrawChart(v, this.selectVom);
          }}
        >
          {stocks.map((i) => (
        ...
        <Select
          ...
          defaultValue={40000}
          onChange={(v) => {
            self.setState({ selectVom: v });
            self.reDrawChart(this.state.selectStockId, v);
          }}
        >
          {vols.map((i) => (
  ...
  reDrawChart(stock_id, minvol = 40000) {
    var self = this;
    fetch(`/stock_info?stock_id=${stock_id}&minvol=${minvol}`, {
      method: 'GET',
    })
      .then((res) => res.json())
      .then((data) => {
