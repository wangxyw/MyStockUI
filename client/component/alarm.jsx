...
export const AlarmComponent = () => {
    ...
    (data, selectConsDays, selectConsTotal, selectConsUpDown) => {
      ...
      data &&
        data.forEach((i) => {
          ...
          fetch(
            `/stock_alarm?stock_id=${symbol}&alarm_type=${selectAlarmType}&date_str=${getBeforeDate(
              selectDays
            )}`,
            { method: 'GET' }
          )
  ...
  const advancedSearch = useCallback(
    (selectConsDays, selectConsTotal, selectConsUpDown) => {
      const upDownStocks = [];
      fetch(`/all_alarm_data?date_str=${getBeforeDate(selectDays)}`, {
        method: 'GET',
      })
        .then((res) => res.json())
        .then((result) => {
  ...
  const getStockAlarm = useCallback(() => {
    validateStock(selectStock) &&
      fetch(
        `/stock_alarm?stock_id=${selectStock}&alarm_type=${selectAlarmType}`,
        { method: 'GET' }
      )
        .then((res) => res.json())
        .then((data) => {
    ...
    fetch(
      `/update_stock_status?stock_id=${selectStock}&datestr=${moment(
        new Date()
      ).format(dateFormat)}`,
      { method: 'GET' }
    )
  ...
  const addtoFocus = useCallback(() => {
    fetch(
      `/add_focus?stock_id=${selectStock}&datestr=${selectDate}&comments=${comments}&predict=${predict}`,
      { method: 'GET' }
    ).then((res) => res.json());
